...
function renderizarProductos() {
  ...
  // Productos sin material (Drive-sync) solo aparecen en "todos"
  let filtrados = materialActivo === "todos"
    ? [...productosActivos]
    : productosActivos.filter(p => p.material === materialActivo);
...
function actualizarSubcategorias() {
  ...
  if (materialActivo === "todos" || materialActivo === "accesorio") {
    ...
    subcatBtns.forEach(btn => {
      ...
      const hay = productosActivos.some(p => p.material === materialActivo && p.categoria === subcat);
